...
const ShoppingList = () => {
  ...
  const fetchShoppingList = async () => {
    try {
      ...
      //Grouped Format to flat array for easier manipulation
      const flatItems = [];
  ...
  const handleToggleChecked = async (id) => {
    ...
    try {
      ...
      // 3. If the DB fails, fetch the true list to revert the UI back
      fetchShoppingList();
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {showAddModal && (
        <AddItemModal
          ...
          onSuccess={() => {
            // ✅ Safest method: just pull the fresh list from the database!
            fetchShoppingList();
